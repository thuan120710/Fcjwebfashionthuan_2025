...
const TABLE_NAME = process.env.COUPONS_TABLE_NAME || "ShopCoupons";
...
function calculateDiscount(coupon, orderTotal) {
  ...
  if (orderTotal < coupon.minimumPurchase) return 0;
  let discount = 0;
...
function isApplicableToProduct(coupon, productId) {
  if (!coupon.applicableProducts || coupon.applicableProducts.length === 0) {
    return (
      !coupon.excludedProducts || !coupon.excludedProducts.includes(productId)
    );
  }
  return (
...
async function createCoupon(data) {
  const coupon = {
    ...
    minimumPurchase: parseFloat(data.minimumPurchase) || 0,
    ...
    userRestriction: data.userRestriction || false,
